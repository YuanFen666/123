...
```ini
[common]
exam_watch = true        ; 每次运行列出所有考试 + 就绪体检
exam_warn_hours = 48     ; 剩余不足 48 小时时每次运行都提醒
```

只调两个**只读**接口：考试列表 + 考试封面页（读门槛/人脸/验证码/监控字段）。
输出形如：
...
**它不会替你进考场，也不会提交任何东西**（体检不带 `redo`/`examsignal`、不跟随跳转、
绝不调用 `phone/start`）。

**为什么不做自动答题**：还差「虚拟 IMEI + 提交签名（签名里塞屏幕点击坐标）+ 滑块验证码」三块，
而且**无法测试** —— 一门考试卡在 90% 任务点门槛进不去，另一门一旦实测就用掉唯一一次机会。
